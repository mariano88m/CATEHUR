...
Public Class MapperEvento
    ...
    Public Function TraerEstados(LaConfig As ConfiguracionConexion) As List(Of EstadoEventoEntity)
        Dim lstEstado As New List(Of EstadoEventoEntity)
        Dim unaconexion As New Conexion(Conexion.MotoresDisponibles.SqlServerClient)
        unaconexion.ConexionIniciar(LaConfig)

        Dim resultado As IDataReader = unaconexion.Ejecutar("Select EstadoEvento.NombreEstadoEvento, EstadoEvento.id_estado from EstadoEvento", False, IConexion.TipoRetorno.Tupla).ResultadoConectado

        lstEstado = DataReadObjeto.RealizarMapeo(Of EstadoEventoEntity)(resultado)
